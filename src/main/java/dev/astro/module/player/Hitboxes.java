...
public final class Hitboxes extends Module {
    ...
    private boolean shouldRender(Entity entity, Minecraft mc) {
        ...
        if (entity instanceof EntityPlayer && ((EntityPlayer) entity).isInvisibleToPlayer(mc.thePlayer)) return false;

        if (entity == mc.thePlayer) {
            return showSelf.getValue() && mc.gameSettings.thirdPersonView != 0;
        }
        if (entity instanceof EntityPlayer) {
            return showPlayers.getValue() && isTrackedPlayer((EntityPlayer) entity, mc);
        }
